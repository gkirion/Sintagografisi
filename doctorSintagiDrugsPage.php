<?php
    ...
    $sql = "insert into sintagi(open_date, expiration_date, visit_id) values(now(), now() + interval $days day, $visit_id)";    
...
<head>
    ...
    <script type="text/javascript">
        function getData() {
            ...
            document.getElementById("days").innerHTML = sessionStorage.getItem("sintagi_days");
            document.getElementById("sintagi_id").value = <?php echo "$last_id" ?>;
            sessionStorage.setItem("sintagi_id", <?php echo "$last_id" ?>);
        }
        ...
        function getRow(btn) {
            ...
            var ids = document.getElementsByName("drug_id[]");
            var quantities = document.getElementsByName("quantity[]");
            var reasons = document.getElementsByName("drug_reason[]");
            for (var i = 0; i < ids.length; i++) {
                table_data = table_data + "<tr id=\"" + i + "\"><td><input id=\"drug_id[]\" name=\"drug_id[]\" value=" + ids[i].value + "></td> \
                ...
                <td><input type=\"text\" id=\"drug_reason[]\" name=\"drug_reason[]\" value=\"" + reasons[i].value + "\" /></td>\n\
                <td><button onclick=\"deleteRow(this)\">Διαγραφή</button></tr>";
            }
            table_data = table_data + "<tr id=\"" + ids.length + "\"><td><input id=\"drug_id[]\" name=\"drug_id[]\" value=" + data + "></td> \n\
            ...
            <td><input type=\"text\" id=\"drug_reason[]\" name=\"drug_reason[]\" /></td>\n\
            <td><button onclick=\"deleteRow(this)\">Διαγραφή</button></tr>";
            document.getElementById("drug_list").innerHTML = table_data;
            rows = rows + 1;
        ...
        function deleteRow(btn) {
            var id = btn.parentElement.parentElement.id;
            var table_data = document.getElementById("drug_list").innerHTML;
        ...
        $(document).ready(function() { 
            $("#drug_table").tablesorter({widgets: ['zebra']}); 
        }
        );
            
        var options = {
            valueNames: [ 'id', 'name', 'manufacturer', 'drastiki_ousia', 'price' ]
        };
...
<table width="100%">
    ...
    <tr><td>
<table><tr><th><h3>Συνταγή</h3></th></tr>
<tr><td>Έγκυρη για (ημέρες): <output id ="days" name="days"></output></td></tr></table>
</td>
<td><table>
...
<thead>
<tr><th class="header">Id</th><th class="header">Όνομα</th><th class="header">Παρασκευαστής</th><th class="header">Δραστική ουσία</th><th class="header">Τιμή</th><th class="header">Ενέργεια</th></tr>
</thead>
<tbody class="list">
<?php 
    $servername = "localhost";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row) {
            ...
            $drastiki_ousia = $row["drastiki_ousia"];
            $price = $row["price"];
            $line = "";
            if ($index % 2) {
                $line = "odd";
            ...
            echo "<tr id=\"row$index\" class=\"$line\"><td>$id</td><td>$name</td><td>$manufacturer</td><td>$drastiki_ousia</td><td>$price</td><td><button onclick=\"getRow(this)\">Εισαγωγή</button></td></tr>";
            $index++;
            $row = mysqli_fetch_assoc($result);
        }
    }
...
<td>
<form action="http://snf-655727.vm.okeanos.grnet.gr/PhpProject1/doctorCompleteSintagi.php" method="post">
